<script>
  ...
  onMount(async () => {
    ...
    await loadData(); // ✅ Esperar a que termine de cargar
    addItem(); // ✅ Ahora sí hay medicamentos disponibles
  });
  ...
  function addItem() {
    // ✅ Verificar que hay medicamentos antes de agregar
    if (!medicamentos || medicamentos.length === 0) {
    ...
    items = [...items, { 
      id: Date.now(), 
      medicamentoId: medicamentos[0].id, // ✅ Ahora sí existe
      cantidad: 1 
    }];
    calcTotal();
  }
  ...
  <div class="bg-white rounded-lg shadow-lg overflow-hidden">
    ...
    <div class="p-6 space-y-6">
      ...
      {:else if medicamentos.length === 0}
        <div class="text-center py-8">
          <p class="text-red-600">⚠️ No hay medicamentos disponibles</p>
        </div>
      {:else}
        <!-- Selector de Cliente -->
